...
"""
测试TransactionService类的添加交易记录功能
"""
...
def test_add_transaction():
    ...
    try:
        # 调用TransactionService处理交易
        success, result, status_code = TransactionService.process_transaction(
            db=db,
            user_id=user_id,  # 使用用户ID
            transaction_data=transaction_data
        )
        
        logger.info(f"处理结果: success={success}, result={result}, status_code={status_code}")
        ...
        if success:
            ...
            if transaction_id:
                ...
                if transaction:
                    ...
                    # 查询分单记录
                    split_query = "SELECT * FROM transaction_splits WHERE original_transaction_id = %s"
                    splits = db.fetch_all(split_query, [transaction_id])
                    
                    if splits:
                        logger.info(f"分单记录已成功插入: {splits}")
                    else:
                        logger.error("分单记录未成功插入")
                else:
